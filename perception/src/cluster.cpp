...
std::vector< std::vector<cv::Point> > Cluster::cluster(cv::Mat &Normalized_time_image, cv::Mat& Time_image, image_transport::Publisher output_pub)
{
    ...
    dbscan.Run(&clusters, 2, threshold, MinPts, distance);
    ...
    Time_image = Time_image*255;
    Time_image.convertTo(Time_image, CV_8UC1);

    //plot output
    if(output.size() != 0)
    {
        for(int numout = 0; numout<output.size(); numout++)
        {
            cv::RotatedRect rect = cv::minAreaRect(output[numout]);
            cv::Point2f ver[4];
            rect.points(ver);
            for(int i = 0; i < 4; i++)
                cv::line(Time_image, ver[i], ver[(i+1)%4], cv::Scalar(255), 5);
        }
    }

    sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "mono8",Time_image).toImageMsg();
