...
Renderer::Renderer(ros::NodeHandle & nh, ros::NodeHandle nh_private,int sample_rate) : nh_(nh),SampleRate(sample_rate)
{
  ...
  image_pub_ = it_.advertise("dvs_redblue", 1);
  sampimg_pub_ = it_.advertise("dvs_2value", 1);
...
void Renderer::eventsCallbackRB(const dvs_msgs::EventArray::ConstPtr& msg)
{
  ...
  {
    cv_bridge::CvImage cv_image;
    cv_bridge::CvImage v2_image;
    if (msg->events.size() > 0)
    {
      cv_image.header.stamp = msg->events[msg->events.size()/2].ts;
      v2_image.header.stamp = msg->events[msg->events.size()/2].ts;
    }
    ...
    else
    {
      ...
      for (int i = 0; i < msg->events.size(); ++i)
      {
        ...
        // scale image
      cv::normalize(on_events, on_events, 0, 255, cv::NORM_MINMAX, CV_8UC1);
      cv::normalize(off_events, off_events, 0, 255, cv::NORM_MINMAX, CV_8UC1);

      cv_image.image += on_events;
      cv_image.image += off_events;
      cv::threshold(cv_image.image,cv_image.image,0,255,CV_THRESH_BINARY);
    }
...
void Renderer::eventsCallback(const dvs_msgs::EventArray::ConstPtr& msg){
    eventsCallbackRB(msg);
    if (event_count < SampleRate){
        event_count++;
    }
    else {
        eventsCallback2VwithRate(msg);
        event_count = 0;
    }
}

void Renderer::eventsCallback2VwithRate(const dvs_msgs::EventArray::ConstPtr& msg ){
